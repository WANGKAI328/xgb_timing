...
def plot_trend_scatter(
    ...
    """
    创建带有按钮切换功能的趋势散点图，可显示所有点、上涨趋势点或下跌趋势点
    
    参数:
        df (pd.DataFrame): 包含数据的DataFrame
        x_col (str): X轴数据列名，默认为'trade_day'
        y_col (str): Y轴数据列名，默认为'close'
        t_col (str): 用于确定上涨/下跌趋势的列名，默认为't_value'
        color_col (str): 用于点颜色的列名，默认为'normed_t_value'
        label_col (str, optional): 用于区分上涨/下跌趋势的列名，如果为None，则尝试使用'label'列或根据t_col自动创建
        hover_cols (List[str], optional): 鼠标悬停时显示的列，如果为None，则显示index和y_col
        colorscale (str): 颜色映射，默认为'RdBu'
        title (str): 图表标题
        plot_width (int): 图表宽度，默认1000
        plot_height (int): 图表高度，默认600
        point_size (int): 散点大小，默认8
        save (bool): 是否保存图表为HTML文件
        save_path (str, optional): 保存路径
        auto_open (bool): 是否自动打开保存的HTML文件
    ...
    # 复制DataFrame避免修改原始数据
    df_plot = df.copy()
    df_plot[x_col] = df_plot[x_col].apply(lambda time: time.strftime("%Y-%m-%d"))
    
    # 如果没有提供label_col，但存在'label'列，则使用'label'列
    if label_col is None and 'label' in df_plot.columns:
        label_col = 'label'
    # 如果没有label_col和'label'列，则根据t_col创建label
    elif label_col is None:
        if t_col in df_plot.columns:
            df_plot['label'] = np.sign(df_plot[t_col])
            label_col = 'label'
        else:
            raise ValueError(f"需要提供label_col或确保DataFrame中存在'label'列或{t_col}列")
    
    # 准备自定义hover数据
    if hover_cols is None:
        hover_cols = [x_col, y_col]
        if t_col in df_plot.columns and t_col != y_col:
            hover_cols.append(t_col)
        if color_col in df_plot.columns and color_col != y_col and color_col != t_col:
            hover_cols.append(color_col)
        
        # 检查fullt_tested列是否存在
        if 'fullt_tested' in df_plot.columns:
            hover_cols.append('fullt_tested')
    
    # 转换hover_cols为array便于customdata使用
    customdata = np.stack([df_plot[col] if col in df_plot.columns else df_plot.index for col in hover_cols], axis=-1)
    
    # 创建hovertemplate
    hovertemplate = ''
    for i, col in enumerate(hover_cols):
        if col == df_plot.index.name or col == 'index':
            hovertemplate += f'<b>日期</b>: %{{customdata[{i}]}}<br>'
        elif col == x_col:
            hovertemplate += f'<b>日期</b>: %{{customdata[{i}]}}<br>'
        elif col == y_col:
            hovertemplate += f'<b>价格</b>: %{{y:.2f}}<br>'
        elif col == t_col:
            hovertemplate += f'<b>t值</b>: %{{customdata[{i}]:.4f}}<br>'
        # elif col == color_col:
        #     hovertemplate += f'<b>归一化t值</b>: %{{customdata[{i}]:.4f}}<br>'
        else:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode='markers',
            marker=dict(
                color=df_plot[color_col] - 0.5 if color_col in df_plot.columns else None,
                colorscale=colorscale,
                cmin=-1,
                cmax=1,
                size=point_size,
                showscale=True,
                colorbar=dict(title='归一化t值')
            ),
    ...
    # 添加上涨趋势的散点图
    up_trend = df_plot[df_plot[label_col] == 1]
    if not up_trend.empty:
        up_customdata = np.stack([up_trend[col] if col in up_trend.columns else up_trend.index for col in hover_cols], axis=-1)
        fig.add_trace(
            go.Scatter(
                x=up_trend[x_col],
                y=up_trend[y_col],
                mode='markers',
                marker=dict(
                    color=up_trend[color_col] - 0.5 if color_col in up_trend.columns else None,
                    colorscale=colorscale,
                    cmin=-1,
                    cmax=1,
                    size=point_size,
    ...
    # 添加下跌趋势的散点图
    down_trend = df_plot[df_plot[label_col] == -1]
    if not down_trend.empty:
        down_customdata = np.stack([down_trend[col] if col in down_trend.columns else down_trend.index for col in hover_cols], axis=-1)
        fig.add_trace(
            go.Scatter(
                x=down_trend[x_col],
                y=down_trend[y_col],
                mode='markers',
                marker=dict(
                    color=down_trend[color_col] - 0.5 if color_col in down_trend.columns else None,
                    colorscale=colorscale,
                    cmin=-1,
                    cmax=1,
                    size=point_size,
